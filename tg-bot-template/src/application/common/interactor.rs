...
//!
//! The template follows a "one handler - one interactor" convention: each bot
//! handler delegates its business operation to a dedicated interactor instead of
//! embedding application logic in the handler. This keeps handlers focused on
//! transport concerns such as update extraction and Telegram responses.
//!
//! Interactors are created by DI, so infrastructure-facing dependencies such as
//! `TxManager` belong on the interactor itself. `Input` should contain update
//! data for the operation, not dependencies that the container can provide.

/// Executes one application operation for a handler or another caller.
///
...
/// domain/application error type that the handler can map into a response.
